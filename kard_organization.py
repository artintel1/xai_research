...
def organize_kard_dataset(source_dir, target_dir, split_ratios=(0.7, 0.15, 0.15), dry_run=False):
    """
    Reorganizes the KARD_video dataset from a flat structure of action folders
    ...
    Args:
        source_dir (str or Path): The path to the original KARD_video dataset.
    ...
    for action_class_path in action_classes:
        ...
        if not dry_run:
            (train_dir / action_name).mkdir()
            (val_dir / action_name).mkdir()
            (test_dir / action_name).mkdir()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Reorganize the KARD video dataset into train/val/test splits.",
    ...
    parser.add_argument(
        '--source_dir',
        type=str,
        default='data/KARD_video',
        help='The source directory of the KARD dataset.'
    )
    parser.add_argument(
        '--target_dir',
        type=str,
        default='data/KARD_video_organized',
